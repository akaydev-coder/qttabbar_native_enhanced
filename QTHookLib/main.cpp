...
{
    for (BitmapGDI* image : m_config.imageList) {
        delete image;
    }
...
std::wstring GetIniString(std::wstring FilePath, std::wstring AppName, std::wstring KeyName)
{
	if (FileIsExist(FilePath)) {
		std::vector<wchar_t> data(32768, L'\0');
		DWORD length = GetPrivateProfileStringW(AppName.c_str(), KeyName.c_str(), NULL,
				data.data(), static_cast<DWORD>(data.size()), FilePath.c_str());
...
void LoadSettings(bool loadimg)
{
    ...
    std::wstring enabled = GetIniString(cfgPath, L"image", L"enabled");
    if (enabled == L"false" || enabled == L"0") {
        return;
    }
    ...
    */
    ...
    if (loadimg) {
        ...
        if (!imgPath.empty()) {
            ...
            DWORD expandedLength = ExpandEnvironmentStringsW(imgPath.c_str(), expanded, ARRAYSIZE(expanded));
            if (expandedLength > 0 && expandedLength < ARRAYSIZE(expanded)) {
                imgPath.assign(expanded);
            }
            if (PathIsRelativeW(imgPath.c_str())) {
                imgPath = GetCurDllDir() + L"\\" + imgPath;
            }
        }
...
BitmapGDI::BitmapGDI(std::wstring path)
    : pMem(nullptr), pBmp(nullptr), Size{}, src(nullptr)
{
	//这样加载是为了防止文件被占用
	FILE* file = nullptr;
	...
	if (file) {
		...
		if ( true )
		{
			...
			if (src && src->GetLastStatus() == Gdiplus::Ok) {
				...
				if (src->GetHBITMAP(0, &pBmp) == Gdiplus::Ok && pMem && pBmp) {
...
extern "C" __declspec(dllexport) int InitializeBackground(CallbackStruct* cb);
...
int RegisterBackgroundWindow(HWND hwnd) {
    ...
    if (wcscmp(className, L"DirectUIHWND") != 0) return E_INVALIDARG;

    DWORD threadId = GetWindowThreadProcessId(hwnd, nullptr);
    if (threadId == 0) return E_FAIL;

    MyData data = {};
    data.hWnd = hwnd;
    if (m_config.isRandom && !m_config.imageList.empty()) {
        data.ImgIndex = rand() % m_config.imageList.size();
    }
    m_duiList[threadId] = data;
...
)
{
    ...
    if (hWnd)
    {
		GetClassName(hWnd, cn_chars, ARRAYSIZE(cn_chars));
		GetClassName(hWndParent, pcn_chars, ARRAYSIZE(pcn_chars));
        ...
        && parentClassName == L"SHELLDLL_DefView")
    {
		...
		GetClassName(parent, cn_chars, ARRAYSIZE(cn_chars));
        classname = std::wstring(cn_chars);
        ...
        if(SUCCEEDED(psb->GetWindow(&hwnd))) {
            ...
            if(SendMessage(parent, WM_CHECKPULSE, NULL, (WPARAM)(&pdisp)) != 0 && pdisp != NULL) {
                ...
                if(psw.Create(CLSID_ShellWindows, CLSCTX_ALL)) {
                    long cookie;
                    if(SUCCEEDED(psw->Register(pdisp, HandleToLong(hwnd), SWC_EXPLORER, &cookie))) {
                        psw->Revoke(cookie);
                    }
                }
            }
