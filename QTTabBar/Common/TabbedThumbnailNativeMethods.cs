...
namespace QTTabBarLib.Common
{
    internal static class TabbedThumbnailNativeMethods
    {
        ...
        internal const int DisallowPeek = 11;
        internal const int ExcludedFromPeek = 12;
        ...
        internal static void SetWindowPeekPolicy(IntPtr hwnd, bool disallowPeek, bool excludedFromPeek)
        {
            SetBooleanWindowAttribute(hwnd, DisallowPeek, disallowPeek);
            SetBooleanWindowAttribute(hwnd, ExcludedFromPeek, excludedFromPeek);
        }

        private static void SetBooleanWindowAttribute(IntPtr hwnd, uint attribute, bool enabled)
        {
            var value = Marshal.AllocHGlobal(4);
            Marshal.WriteInt32(value, enabled ? 1 : 0);
            try
            {
                var rc = DwmSetWindowAttribute(hwnd, attribute, value, 4);
                if (rc != 0)
                {
                    throw Marshal.GetExceptionForHR(rc);
                }
