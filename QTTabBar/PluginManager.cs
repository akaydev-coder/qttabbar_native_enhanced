...
namespace QTTabBarLib {
    internal static class PluginManager {
        ...
        private static class NativePluginInterop
        {
            ...
            public static List<NativePluginMetadata> ReadMetadata(bool reloadLibraries)
            {
        ...
        private static void RefreshNativeMetadataCache(bool reloadLibraries)
        {
            cachedNativeMetadata = NativePluginInterop.ReadMetadata(reloadLibraries);
        ...
        public static void Initialize() {
            RefreshNativeMetadataCache(true);
        ...
        private static IEnumerable<string> ReadAssemblyPaths() {
            if(cachedNativeMetadata.Count > 0) {
                foreach(var metadata in cachedNativeMetadata) {
                    if(metadata.ManagedFallback && !string.IsNullOrEmpty(metadata.Path) && File.Exists(metadata.Path)) {
 ...
 */
        ...
        internal static void ApplyNativeEnablement(IEnumerable<string> enabledPluginIds) {
            ...
            UpdateCachedNativeEnablement(enabledSet);
        }

        private static void UpdateCachedNativeEnablement(IEnumerable<string> enabledPluginIds) {
            if(enabledPluginIds == null) {
                return;
            }

            HashSet<string> enabledSet = enabledPluginIds as HashSet<string>
                    ?? new HashSet<string>(enabledPluginIds, StringComparer.OrdinalIgnoreCase);
            foreach(var key in dicNativePluginMetadata.Keys.ToList()) {
                var metadata = dicNativePluginMetadata[key];
                metadata.Enabled = enabledSet.Contains(key);
                dicNativePluginMetadata[key] = metadata;
            }
