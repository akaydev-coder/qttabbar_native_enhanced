...
namespace QTTabBarLib {
    ...
    public partial class QTTabBarClass : TabBarBase
    {
        ...
        private Timer timerFluentGlassRefresh;
        private Timer timerTaskbarSnapshot;
        ...
        private TaskbarTabPreviewManager taskbarTabPreviews;
        private readonly Dictionary<QTabItem, Bitmap> taskbarTabPreviewSnapshots = new Dictionary<QTabItem, Bitmap>();
        ...
        public QTTabBarClass() {
            ...
            // ��ʼ���߶�
            BandHeight = Config.Skin.TabHeight + (FluentGlassManager.Enabled ? 0 : BandHeightSpace);
        ...
        public override void CloseDW(uint dwReserved) {
            try {
                DisposeFileListFocusTimer();
                DisposeFluentGlassRefreshTimer();
                DisposeTaskbarSnapshotTimer();
 ...
 */
                ...
                if(IsShown) {
                    ...
                    if(taskbarTabPreviews != null) {
                        taskbarTabPreviews.Dispose();
                        taskbarTabPreviews = null;
                    }
                    ClearTaskbarTabPreviewSnapshots();
                }
                if(TravelLog != null) {
        ...
        protected override void Dispose(bool disposing) {
            if(disposing) {
                DisposeFluentGlassRefreshTimer();
                DisposeTaskbarSnapshotTimer();
                FluentGlassManager.Disable(ExplorerHandle);
                if(components != null) {
                    components.Dispose();
                }
            }
        ...
        private void Explorer_NavigateComplete2(object pDisp, ref object URL) {
            ...
            if(fNowQuitting)
            {
                ...
                try {
                    ...
                    if(focusFileListAfterNavigation || fNowTravelByTree || !FirstNavigationCompleted) {
                        focusFileListAfterNavigation = false;
        ...
        public override void GetBandInfo(uint dwBandID, uint dwViewMode, ref DESKBANDINFO dbi) {
            ...
            if((dbi.dwMask & DBIM.MODEFLAGS) != (0)) {
                dbi.dwModeFlags = FluentGlassManager.Enabled ? DBIMF.NOMARGINS | DBIMF.NOGRIPPER : DBIMF.NORMAL;
            }
        ...
        private void InitializeComponent() {
            ...
            MinSize = new Size(150, Config.Skin.TabHeight + (FluentGlassManager.Enabled ? 0 : 2));
            Height = Config.Skin.TabHeight + (FluentGlassManager.Enabled ? 0 : 2);
        ...
        private void InitializeOpenedWindow() {
            if(IsShown && listViewManager != null) return;
            IsShown = true;
            InstanceManager.PushTabBarInstance(this);
            //  ��װ����
            ...
            Timer restoreVerticalBarTimer = new Timer { Interval = 1000 };
            restoreVerticalBarTimer.Tick += (sender, args) => {
                restoreVerticalBarTimer.Stop();
                restoreVerticalBarTimer.Dispose();
                if(!fClosedDW) QCommandBarVertical.RestoreForExplorer(Explorer);
            };
            restoreVerticalBarTimer.Start();
        ...
        internal void RefreshOptions() {
            ...
            // Reapplying an already active DWM frame during every options commit
            // briefly exposes Explorer's black/solid fallback background. Only
            // attach a missing frame or disable one that the user switched off.
            if(!FluentGlassManager.IsAppliedTo(ExplorerHandle) || !FluentGlassManager.Enabled) {
                ApplyFluentGlass();
            }
            foreach(QTabItem item in tabControl1.TabPages) {
                item.RefreshRectangle();
        ...
        private void RefreshTaskbarTabPreviews() {
            RefreshTaskbarTabPreviews(false);
        }

        private void RefreshTaskbarTabPreviews(bool updateSnapshot) {
            try {
                if(updateSnapshot) {
                    ScheduleTaskbarTabPreviewSnapshot();
                }
                if(taskbarTabPreviews == null) {
                    taskbarTabPreviews = new TaskbarTabPreviewManager(
                        index => {
                        ...
                        index => {
                            if(index >= 0 && index < tabControl1.TabCount) {
                                QTabItem tabToClose = tabControl1.TabPages[index];
                                try {
                                    BeginInvoke(new Action(() => {
                                        if(IsDisposed || tabControl1 == null || tabControl1.IsDisposed ||
                                                !tabControl1.TabPages.Contains(tabToClose)) {
                                            return;
                                        }
                                        if(tabControl1.TabCount == 1) {
                                    ...
                                    QTUtility2.MakeErrorLog(ex, "Taskbar thumbnail close dispatch");
                                }
                            }
                        });
                }

                List<TaskbarTabPreviewManager.Entry> entries = new List<TaskbarTabPreviewManager.Entry>();
                List<QTabItem> liveTabs = new List<QTabItem>();
                for(int i = 0; i < tabControl1.TabCount; i++) {
                    QTabItem tab = tabControl1.TabPages[i];
                    liveTabs.Add(tab);
                    Icon icon = GetTaskbarTabShellIcon(tab);
                    entries.Add(new TaskbarTabPreviewManager.Entry {
                        Title = String.IsNullOrEmpty(tab.Text) ? tab.CurrentPath : tab.Text,
                        Path = tab.CurrentPath,
                        Icon = icon,
                        Preview = GetTaskbarTabPreviewSnapshot(tab),
                        Active = i == tabControl1.SelectedIndex
                    });
                }
                PruneTaskbarTabPreviewSnapshots(liveTabs);
                taskbarTabPreviews.Update(ExplorerHandle, true, entries);
                foreach(TaskbarTabPreviewManager.Entry entry in entries) {
                    if(entry.Icon != null) {
                        entry.Icon.Dispose();
                    }
                }
        ...
        private static Icon GetTaskbarTabShellIcon(QTabItem tab) {
            ...
            try {
                ...
                try {
                    if(tab.CurrentIDL != null && tab.CurrentIDL.Length > 0) {
                        wrapper = new IDLWrapper(tab.CurrentIDL);
                    }
                    if(wrapper == null || !wrapper.Available) {
                        if(wrapper != null) wrapper.Dispose();
                        wrapper = new IDLWrapper(tab.CurrentPath);
                    }
                    if(wrapper == null || !wrapper.Available) return null;

                    SHFILEINFO info = new SHFILEINFO();
                    IntPtr result = PInvoke.SHGetFileInfo(wrapper.PIDL, 0, ref info,
                            Marshal.SizeOf(typeof(SHFILEINFO)), 0x109);
                ...
                QTUtility2.MakeErrorLog(ex, "GetTaskbarTabShellIcon");
                return null;
            }
        }

        private bool CaptureTaskbarTabPreviewSnapshot() {
            if(CurrentTab == null || ExplorerHandle == IntPtr.Zero || PInvoke.IsIconic(ExplorerHandle) || !PInvoke.IsWindowVisible(ExplorerHandle)) {
                return false;
            }
            IntPtr foreground = PInvoke.GetForegroundWindow();
            if(foreground != ExplorerHandle && !PInvoke.IsChild(ExplorerHandle, foreground)) {
                return false;
            }
            ...
            try {
                screenBitmap = new Bitmap(bounds.Width, bounds.Height, PixelFormat.Format32bppArgb);
                using(Graphics g = Graphics.FromImage(screenBitmap)) {
                    g.CopyFromScreen(bounds.Left, bounds.Top, 0, 0, bounds.Size, CopyPixelOperation.SourceCopy);
                }

                const int MaxSnapshotWidth = 1920;
                const int MaxSnapshotHeight = 1080;
                double scale = Math.Min(1.0, Math.Min(
                        (double)MaxSnapshotWidth / bounds.Width,
                        (double)MaxSnapshotHeight / bounds.Height));
                Size snapshotSize = new Size(
                        Math.Max(1, (int)Math.Round(bounds.Width * scale)),
                        Math.Max(1, (int)Math.Round(bounds.Height * scale)));
                bitmap = new Bitmap(snapshotSize.Width, snapshotSize.Height, PixelFormat.Format32bppArgb);
                using(Graphics g = Graphics.FromImage(bitmap)) {
                    g.InterpolationMode = System.Drawing.Drawing2D.InterpolationMode.HighQualityBicubic;
                    g.PixelOffsetMode = System.Drawing.Drawing2D.PixelOffsetMode.HighQuality;
                    g.DrawImage(screenBitmap, new Rectangle(Point.Empty, snapshotSize));
                }

                Bitmap old;
                if(taskbarTabPreviewSnapshots.TryGetValue(CurrentTab, out old)) {
                    old.Dispose();
                }
                taskbarTabPreviewSnapshots[CurrentTab] = bitmap;
                bitmap = null;
                return true;
            }
            catch(Exception ex) {
                QTUtility2.MakeErrorLog(ex, "CaptureTaskbarTabPreviewSnapshot");
                return false;
            }
            finally {
        ...
        private void ScheduleTaskbarTabPreviewSnapshot() {
            if(IsDisposed || !IsHandleCreated) {
                return;
            }
            if(timerTaskbarSnapshot == null) {
                timerTaskbarSnapshot = new Timer { Interval = 450 };
                timerTaskbarSnapshot.Tick += timerTaskbarSnapshot_Tick;
            }
            timerTaskbarSnapshot.Stop();
            timerTaskbarSnapshot.Start();
        }

        private void timerTaskbarSnapshot_Tick(object sender, EventArgs e) {
            timerTaskbarSnapshot.Stop();
            if(CaptureTaskbarTabPreviewSnapshot()) {
                RefreshTaskbarTabPreviews(false);
            }
        }

        private void DisposeTaskbarSnapshotTimer() {
            if(timerTaskbarSnapshot == null) {
                return;
            }
            timerTaskbarSnapshot.Stop();
            timerTaskbarSnapshot.Tick -= timerTaskbarSnapshot_Tick;
            timerTaskbarSnapshot.Dispose();
            timerTaskbarSnapshot = null;
        }

        private Bitmap GetTaskbarTabPreviewSnapshot(QTabItem tab) {
            Bitmap bitmap;
            return tab != null && taskbarTabPreviewSnapshots.TryGetValue(tab, out bitmap) ? bitmap : null;
        }

        private void PruneTaskbarTabPreviewSnapshots(IList<QTabItem> liveTabs) {
            List<QTabItem> remove = taskbarTabPreviewSnapshots.Keys.Where(tab => !liveTabs.Contains(tab)).ToList();
            foreach(QTabItem tab in remove) {
                taskbarTabPreviewSnapshots[tab].Dispose();
                taskbarTabPreviewSnapshots.Remove(tab);
            }
        }

        private void ClearTaskbarTabPreviewSnapshots() {
            foreach(Bitmap bitmap in taskbarTabPreviewSnapshots.Values) {
                bitmap.Dispose();
            }
            taskbarTabPreviewSnapshots.Clear();
        }
        ...
        private void ApplyFluentGlass() {
            if(ExplorerHandle != IntPtr.Zero) {
                FluentGlassManager.Apply(ExplorerHandle, Handle);
            }
        }

        private void ScheduleFluentGlassRefresh() {
            if(!FluentGlassManager.Enabled || IsDisposed || !IsHandleCreated) {
                return;
            }

            if(timerFluentGlassRefresh == null) {
                timerFluentGlassRefresh = new Timer { Interval = 60 };
                timerFluentGlassRefresh.Tick += timerFluentGlassRefresh_Tick;
            }

            timerFluentGlassRefresh.Stop();
            timerFluentGlassRefresh.Start();
        }

        private void timerFluentGlassRefresh_Tick(object sender, EventArgs e) {
            timerFluentGlassRefresh.Stop();
            if(IsDisposed || !IsHandleCreated || !PInvoke.IsWindow(ExplorerHandle)) {
                return;
            }

            ApplyFluentGlass();
            QTUtility2.log("FluentGlass deferred refresh after navigation");
        }

        private void DisposeFluentGlassRefreshTimer() {
            if(timerFluentGlassRefresh == null) {
                return;
            }
            ...
            timerFluentGlassRefresh.Stop();
            timerFluentGlassRefresh.Tick -= timerFluentGlassRefresh_Tick;
            timerFluentGlassRefresh.Dispose();
            timerFluentGlassRefresh = null;
        }
        ...
        */
        ...
        protected void SetBarRows(int count)
        {
            ...
            BandHeight = (count * Config.Skin.TabHeight) + (FluentGlassManager.Enabled ? 0 : BandHeightSpace);
